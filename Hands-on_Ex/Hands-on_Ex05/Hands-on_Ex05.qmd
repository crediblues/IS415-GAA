...
In this hands-on exercise, we will be computing spatial weights using R. The following objectives will be achieved
...
We would load spdep, sf, tmap and tidyverse packages into our R environment.
...
## 5.3 Creating (ROOK) contiguity based neighbours
...
There are 88 area units in Hunan. The most connect area unit has 10 neighbours. There are two area units with only one heighbours
...
### 5.2.1 Plotting Queen contiguity based neighbours map
...
In this section, we would derive distance-based weight matrices by using dnearneigh() of spdep package.
...
The "Average number of links: 3.681818" refers to the average number of neighbors (connections or links) that each region has within the specified distance range of 0 to 62 units (which could be kilometers, depending on the coordinate system being used). Specifically, this means that, on average, each of the 88 regions in the dataset has approximately 3.68 neighboring regions that are located within the 62-unit distance. This is computed by taking the total number of links (324) and dividing it by the number of regions (88), resulting in the average number of links per region.
...
Each county has six neighbours. \### 6.3.1 Plotting distance based neighbours We can plot the weight matrix using the code chunk below.
...
In this section, we would create four different spatial lagged variables, they are:
...
8.9.3 Spatial window average The spatial window average uses row-standardized weights and includes the diagonal element. To do this in R, we need to go back to the neighbors structure and add the diagonal element before assigning weights.
