...
wm_d62
```
The "Average number of links: 3.681818" refers to the average number of neighbors (connections or links) that each region has within the specified distance range of 0 to 62 units (which could be kilometers, depending on the coordinate system being used). Specifically, this means that, on average, each of the 88 regions in the dataset has approximately 3.68 neighboring regions that are located within the 62-unit distance.
This is computed by taking the total number of links (324) and dividing it by the number of regions (88), resulting in the average number of links per region.
...
str(knn6)
```
Each county has six neighbours.
