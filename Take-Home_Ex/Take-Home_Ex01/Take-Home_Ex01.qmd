...
```
In the above plot, the plot is too small and the values are low and difficult to interpret.
```{r}
kde_quarterly_ppp_mmr1_ppl <- density(quarterly_ppp_mmr[[1]], 
                               sigma=bw.ppl, 
                               edge=TRUE,
                               kernel="gaussian")

plot(kde_quarterly_ppp_mmr1_ppl, main = "bw.ppl")
```
