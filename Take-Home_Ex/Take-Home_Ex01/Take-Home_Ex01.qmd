...
```{r}
pacman::p_load(png, sp, sf, raster, spatstat, tmap, tidyverse, lubridate)
```
...
Regions: Sagaing, Magway, Mandalay, Shan (North), Chin
...
regions_states <- c("Shan (North)", "Kayah", "Kachin", "Sagaing", "Magway", "Mandalay", "Chin")

# Filter quarterly data to include only the specified regions and states
filter_conflict_data <- function(sf_data) {
  sf_data %>%
    filter(admin1 %in% regions_states | admin2 %in% regions_states)
}
...
F_quarterly_ppp_mmr_envelope <- lapply(filtered_quarterly_ppp, function(ppp_object) {
  envelope(ppp_object, Fest, nsim = 200)
})
```
```{r}
...
Before that, we need to create a new owin object.
```
filtered_quarterly_owin <- lapply(filtered_quarterly_ppp, function(ppp_object) {
  ppp_object[admin_boundary_owin]
})

# Summary of each subsetted ppp object
lapply(filtered_quarterly_owin, summary)
```
```
plot(filtered_quarterly_owin[[3]])
```
